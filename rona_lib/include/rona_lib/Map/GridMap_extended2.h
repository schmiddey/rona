...
#ifndef GRIDMAP_EXTENDED2_H_
#define GRIDMAP_EXTENDED2_H_
 ...
 */
class GridMap_extended2: public GridMap
{
public:
   ...
   virtual std::vector<Node> getChildNodes(unsigned int id)
   {
      ...
      tmp_vec[4]  = (Pixel(p.x  , p.y-2));
      tmp_vec[5]  = (Pixel(p.x,   p.y+2));
      tmp_vec[6]  = (Pixel(p.x+2, p.y  ));
      tmp_vec[7]  = (Pixel(p.x-2, p.y  ));
...
private:
   ...
   void init_extended()
   {
      ...
      _parentDist_extended[4]  = PIXEL_SHORT_SHORT_COST * _grid->getCellSize();
      _parentDist_extended[5]  = PIXEL_SHORT_SHORT_COST * _grid->getCellSize();
      _parentDist_extended[6]  = PIXEL_SHORT_SHORT_COST * _grid->getCellSize();
      _parentDist_extended[7]  = PIXEL_SHORT_SHORT_COST * _grid->getCellSize();
